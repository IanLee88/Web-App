...
# Pre-filled data from my extraction
initial_data = {
...
# Handling the Submit
if submit_button:
    # if merchant or date or currency field is empty, 
    ...
    if not merchant or not date or not currency:
        ...
        if amount <= 0:
            ...
            # if merchant or date or currency field is filled,
            # and if amount is greater than 0, 
            # the merchant, date, amount and currency field is inserted 
            # into the receipt database
            conn = init_db()
            c = conn.cursor()
...
# View Saved Data
# Creating a checkbox with the text "Show saved receipts"
...
# Creating a sidebar button to clear all saved data from the database table
